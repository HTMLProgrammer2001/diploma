    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li><a href="{{route('profile.show')}}">Стажування</a></li>
                <li><a href="{{route('profile.internships.edit', $internship->id)}}">Редагувати</a></li>
            </ol>
        </section>
        ...
        <section class="content">
            ...
            <!-- Default box -->
            <form action="{{route('profile.internships.update', $internship->id)}}"
                                    ...
                                    <select class="form-control select2" id="category" name="category">
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{$category->id}}"
                                                    {{$internship->getCategoryID() == $category->id ? ' selected' : ''}}>
                                    ...
                                    <select class="form-control select2" id="place" name="place">
                                        ...
                                        @foreach($places as $place)
                                            <option value="{{$place->id}}"
                                                    {{$internship->getPlaceID() == $place->id ? ' selected' : ''}}>
                                                {{$place->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
