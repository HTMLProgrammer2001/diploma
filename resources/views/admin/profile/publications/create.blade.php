    ...
    <div class="content-wrapper">
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li><a href="{{route('profile.publications.create')}}">Додати</a></li>
            </ol>
        </section>
        ...
        <section class="content">
            <!-- Default box -->
            <form action="{{route('profile.publications.store')}}" method="post">
                                ...
                                <select class="form-control select2" id="authors" name="authors[]" multiple="multiple"
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{$user->id}}"
                                                {{$user->id == $curUser->id ? ' selected' : ''}}
                                        >{{$user->getShortName()}}</option>
                                    @endforeach
                                </select>
                            </div>
